...
public class PresenterAddMenuItem implements AddMenuItemContract.Presenter{
    ...
    @Override
    public void performAddMenuItem(Context context, Uri uri, String itemName, String itemDescription, String itemPrice) {
        view.showProgress();
            if(uri == null){
                view.validation("Pleas Select the photo");
            }else if(TextUtils.isEmpty(itemName)||TextUtils.isEmpty(itemDescription)||TextUtils.isEmpty(itemPrice)){
                ...
                WebService.getInstance(true).getApi().addManuUserItem(cookie, body, items1, stringValue1,"",itemName,itemDescription,filename,itemPrice).enqueue(new Callback<MainResponse>() {
                    @Override
